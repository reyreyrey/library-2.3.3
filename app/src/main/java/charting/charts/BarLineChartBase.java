...
import charting.interfaces.dataprovider.BarLineScatterCandleBubbleDataProvider;
import charting.interfaces.datasets.IBarLineScatterCandleBubbleDataSet;
import charting.jobs.AnimatedMoveViewJob;
import charting.jobs.AnimatedZoomJob;
import charting.jobs.MoveViewJob;
import charting.jobs.ZoomJob;
...
import charting.utils.MPPointD;
...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends
    ...
    protected void calculateLegendOffsets(RectF offsets) {
        ...
        if (mLegend != null && mLegend.isEnabled() && !mLegend.isDrawInsideEnabled()) {
            if (mLegend.getOrientation() == Legend.LegendOrientation.VERTICAL) {
                ...
                if (mLegend.getHorizontalAlignment() == Legend.LegendHorizontalAlignment.CENTER) {
                    if (mLegend.getVerticalAlignment() == Legend.LegendVerticalAlignment.TOP) {
                        offsets.top += Math.min(mLegend.mNeededHeight,
                                mViewPortHandler.getChartHeight() * mLegend.getMaxSizePercent())
                                + mLegend.getYOffset();
                    }
                    if (mLegend.getVerticalAlignment() == Legend.LegendVerticalAlignment.BOTTOM) {
            ...
            if (mLegend.getOrientation() == Legend.LegendOrientation.HORIZONTAL) {
                if (mLegend.getVerticalAlignment() == Legend.LegendVerticalAlignment.TOP) {
                    offsets.top += Math.min(mLegend.mNeededHeight,
                            mViewPortHandler.getChartHeight() * mLegend.getMaxSizePercent())
                            + mLegend.getYOffset();
